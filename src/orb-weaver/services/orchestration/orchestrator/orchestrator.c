 ...
 * the queue is then processed fifo by the thread handler.
*/
...
bool start_processing(void) {
    ...
    // server runtime - process return means system failure.
...
/*
 * + poll for events on sockets and process on receipt.
*/
static bool poll_events(int socket_descriptor) {
    // server runtime.
    while(1) {
...
/*
 * + pulls data from existing connection and adds task to queue. 
*/
static bool process_data_event(struct kevent *event) {
    ...
    if(current_descriptor < 0) {
        ...
        ERROR_LOG("process_events: Fatal error, unable to fetch socket ID for connection to client.");
        return false;
    }

    if(!enqueue_task(current_descriptor)) {
        ERROR_LOG("process_events: Fatal error, unable to add connection to thread queue.");
        return false;
    }

    // received indicator that the client is finished.
    // final packet may contain data and be waiting on a response.
    if(event->flags & EV_EOF) { 
        ...
        if(!validate_syscall(event->ident, "pull_events", "Failed to close connection.")) {
            ERROR_LOG("close_connection: Error, unable to close connection for: %lu.", event->ident);
            return false;
        };
